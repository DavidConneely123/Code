...
list_of_tensors = tensor_list(20)
...
def Overestimation_of_Vmax_by_Summing(number_of_nuclei):
    my_nuclei = {}
    for i in range(number_of_nuclei):
        name = f'Nuc{i}'
        my_nuclei[name] = my_nuclei.get(name, Nucleus(name = name, spin = 1/2, is_Isotopologue= False, hyperfine_interaction_tensor = list_of_tensors[i]*1e6))
    ...
    Vmax_total = Vmax(0,0,0, display = True)
...
Overestimation_of_Vmax_by_Summing(20)
