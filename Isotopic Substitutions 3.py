...
def Field_dep(number_of_nuclei):
    ...
    for theta_val, phi_val in angles_list:
        ...
        valmin = scipy.sparse.linalg.eigsh(Hspar, k=1, M=None, sigma=None, which='SA', v0=None, ncv=None, maxiter=None,
                                           tol=0, return_eigenvectors=False, Minv=None, OPinv=None)
        Vmax_val = valmax - valmin
        Vmax_val = Vmax_val[0] / 1e6  # Converting Vmax from Hz to MHz

        yum = np.abs(100 * (Vmax_val - Vmax_standard) / Vmax_standard)
...
for nucleus in Nucleus.all[0:6]:
    nucleus.add_to_simulation()

H_hyperfine = H_hyperfine_new()
H_zee = H_zee(0.05,1,1)
H_perp = H_perp(0.05,1,1)
H_tot = H_hyperfine + H_zee
H_dense = H_tot.todense()
Ps = Ps()


eigenvalues, A = scipy.linalg.eigh(H_dense)
A_hermitian_conjugate = scipy.linalg.inv(A)
Ps= A_hermitian_conjugate*Ps*A
H_perp = A_hermitian_conjugate*H_perp*A


eigenvalues = np.divide(eigenvalues, 1e6)

print(eigenvalues)
# eigenvalues[0] is the most negative eigenvalue...


def Histogram_Height(v, dv=0.5):  #NB v in MHz
    sum = 0
    for i in range(4*Nucleus.Nuclear_Dimensions()):
        for j in range(4 * Nucleus.Nuclear_Dimensions()):
            if v - dv/2 < eigenvalues[i] - eigenvalues[j] < v + dv/2:
                t1 = np.abs(H_perp[i,j])**2
                t2 = np.abs(Ps[i,i] - Ps[j,j])
                sum += t1*t2
...
Histogram_Heights = []
for v in range(1, 80):
    print(Histogram_Height(v), v)
    Histogram_Heights.append(Histogram_Height(v))


plt.bar(np.linspace(1, len(Histogram_Heights), len(Histogram_Heights)), Histogram_Heights)
plt.show()