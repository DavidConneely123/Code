...
def tensor_list(number_of_tensors, isotropic = False):
    ...
    for i in range(number_of_tensors):
        ...
        if isotropic:
            symmetric_random_tensors = np.random.uniform(-30, 30) * np.eye(3)
...
# This takes a set of N nuclei, divides them up into a number of subsystems, calculates Vmax for the individual subsystems
# and sums these values, comparing to the Vmax for the full system (NB: the quantity 'subsystem_dividing' decides how the
# system is split up - e.g. with number_of_nuclei = 12 and subsystem_dividing = 2 we get two subsystems each containing 6
# nuclei whereas with number_of_nuclei = 12 and subsystem_dividing = 4 we get one subsystem of 3 nuclei and another sub-
# system of 9 nuclei.

def Overestimation_of_Vmax_by_Summing(number_of_nuclei, subsystem_dividing, isotropic = False, display = False):
    list_of_tensors = tensor_list(number_of_nuclei, isotropic)
    ...
    Vmax_firsthalf = Vmax(0.05,0,0, display=True)
    ...
    Vmax_total = Vmax(0.05,0,0,display=True)
...
def Mean_overestimation(number_of_nuclei, subsystem_dividing, number_of_iterations, isotropic = False):
    i=0
    Overestimation_Sum = 0
    while i<number_of_iterations:
        Overestimation_Sum += Overestimation_of_Vmax_by_Summing(number_of_nuclei, subsystem_dividing, isotropic)
        i += 1
        Nucleus.all = []
